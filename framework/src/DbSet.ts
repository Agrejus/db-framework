...
import { EntityCallbackMany, EntityMap, QueryResult } from "./types";
import { IDbPlugin, NonNullCreateEntity, NonNullEntity, Filter, ParamsFilter } from '@agrejus/db-framework-core';
...
export class DbSet<TEntity extends {}, TEnhancedPropertyNames extends string = never, TComputedPropertyNames extends string = never> {
    ...
    private readonly _manager: IDataAccessManager<TEntity>;

    constructor(dbPlugin: IDbPlugin, schema: CompiledSchema<TEntity>, options: { stateful: boolean }) {
        ...
        if (options.stateful === true) {
            this._manager = new StatefulDataAccessManager<TEntity>(schema, dbPlugin, this.changeTracker);
        } else {
            this._manager = new DataAccessManager<TEntity>(schema, dbPlugin, this.changeTracker);
        }
    }
    ...
    subscribe() {
        const queryable = new Queryable<NonNullEntity<TEntity>, () => void>(null, this._manager as any);
    ...
    where<P extends {} = never>(selector: ParamsFilter<NonNullEntity<TEntity>, P> | Filter<NonNullEntity<TEntity>>, params?: P) {
        ...
        if (params == null) {
            const queryable = new QueryableAsync<NonNullEntity<TEntity>>(null, this._manager as any);
            return queryable.where(selector as Filter<NonNullEntity<TEntity>>);
        }

        const queryable = new ParamsQueryableAsync<NonNullEntity<TEntity>>(null, this._manager as any);
    ...
    sort(selector: EntityMap<TEntity, TEntity[keyof TEntity]>) {
        const result = new QueryableAsync<TEntity>(null, this._manager as any);
    ...
    map<R extends NonNullEntity<TEntity>[keyof NonNullEntity<TEntity>] | Partial<NonNullEntity<TEntity>>>(expression: EntityMap<NonNullEntity<TEntity>, R>) {
        const result = new QueryableAsync<NonNullEntity<TEntity>>(null, this._manager as any);
    ...
    toArray(done: QueryResult<NonNullEntity<TEntity>[]>) {
        const result = new SelectionQueryable<NonNullEntity<TEntity>>(null, this._manager as any);
        return result.toArray(done);
    }

    toArrayAsync(): Promise<NonNullEntity<TEntity>[]> {
        const result = new SelectionQueryableAsync<NonNullEntity<TEntity>>(null, this._manager as any);
