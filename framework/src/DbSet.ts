...
import { OrderedQueryable } from "./query/OrderedQueryable";
import { QueryOrdering } from "./query/types";
import { ShapedQueryable } from "./query/ShapedQueryable";
...
export class DbSet<TEntity extends {}, TEnhancedPropertyNames extends string = never, TComputedPropertyNames extends string = never> {
    ...
    where<P extends {} = never>(selector: ParamsFilter<NonNullEntity<TEntity>, P> | Filter<NonNullEntity<TEntity>>, params?: P) {
        ...
        if (params == null) {
            const queryable = new Queryable<NonNullEntity<TEntity>>(null, this._schema as any, this._dbPlugin);
            queryable.where(selector as Filter<NonNullEntity<TEntity>>);
            return queryable;
        }

        const queryable = new ParamsQueryable<NonNullEntity<TEntity>>(null, this._schema as any, this._dbPlugin);
        queryable.where(selector as ParamsFilter<NonNullEntity<TEntity>, P>, params);
        return queryable;
    }

    order(selector: EntityMap<TEntity, TEntity[keyof TEntity]>) {
        const result = new Queryable<TEntity>(null, this._schema, this._dbPlugin); 
        result.order(selector);
        return result;
    }

    orderDescending(selector: EntityMap<TEntity, TEntity[keyof TEntity]>) {
        const result = new Queryable<TEntity>(null, this._schema, this._dbPlugin); 
    ...
    map<R extends NonNullEntity<TEntity>[keyof NonNullEntity<TEntity>] | Partial<NonNullEntity<TEntity>>>(expression: EntityMap<NonNullEntity<TEntity>, R>) {
        const result = new Queryable<NonNullEntity<TEntity>>(null, this._schema as any, this._dbPlugin); 
