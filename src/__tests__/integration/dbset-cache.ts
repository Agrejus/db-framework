...
describe('DbSet Cache Tests', () => {
    ...
    it('should add entity and cache', async () => {
        ...
        const found = await context.contacts.useCache({ ttl: 10, key: "test" }).find(w => w.firstName === "James");
    ...
    it('should add entity and use cache on second call', async () => {
        ...
        const found = await context.contacts.useCache({ ttl: 10, key: "test" }).find(w => w.firstName === "James");
        const foundCached = await context.contacts.useCache({ ttl: 10, key: "test" }).find(w => w.firstName === "James");
    ...
    it('should add entity and not return cache when ttl has expired', async () => {
        ...
        const found = await context.contacts.useCache({ ttl: 1, key: "test" }).find(w => w.firstName === "James");
        await wait(1100);
        const foundCached = await context.contacts.useCache({ ttl: 10, key: "test" }).find(w => w.firstName === "James");
