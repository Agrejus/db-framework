...
export class PouchDbPlugin<TDocumentType extends string, TEntityBase extends IDbRecord<TDocumentType>, TQueryRequest extends PouchDB.Find.FindRequest<TEntityBase>, TQueryResponse extends PouchDB.Find.FindResponse<TEntityBase>> implements IDbPlugin<TDocumentType, TEntityBase, TQueryRequest, TQueryResponse> {
    ...
    protected createDb() {
        const { dbName, supportsRevisions, ...options } = this._options
    ...
    async query(request: TQueryRequest) {
        return await this.doWork(async w => {

            const response = await w.find(request);

            return response as TQueryResponse;
        })
    }
