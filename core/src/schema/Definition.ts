import { CompiledSchema, InferType, NonNullCreateEntity, NonNullEntity, SchemaTypes } from ".";
...
export class SchemaDefinition<T extends {}> extends SchemaBase<T, any> {
    ...
    private _iterate(instance: SchemaBase<any, any>, callback: (name: string, property: SchemaBase<any, any>, parentPropertyName: string | null, instance: { [key: string]: SchemaBase<any, any> }, parents: { [key: string]: SchemaBase<any, any> }[] | null) => void) {
        const explore: { path: string | null, instance: (SchemaBase<any, any> | { [key: string]: SchemaBase<any, any> }), parents: { [key: string]: SchemaBase<any, any> }[] }[] = [{ path: null, instance, parents: [] }];

        for (let i = 0; i < explore.length; i++) {
            const item = explore[i];

            if (item.instance.type == SchemaTypes.Definition) {
                explore.push({ path: null, instance: item.instance.instance, parents: [] });
                continue;
            }
            ...
            for (const key in instance) {
                ...
                callback(key, property, item.path, instance, [...item.parents]);

                if (property.type === SchemaTypes.Object) {
                    explore.push({ path: [item.path, key].filter(w => w != null).join("."), instance: property.instance, parents: [...item.parents, instance] })
                }
            }
        }
    }

    private _resolvePropertyValue(pathSelector: string, property: SchemaBase<any, any>) {
    ...
    private _appendCloner(property: SchemaBase<any, any>, builder: string[], value: string) {
        builder.push(value);
    }

    private _appendStripper(property: SchemaBase<any, any>, builder: string[], value: string) {
    ...
    private _appendCompare(property: SchemaBase<any, any>, builder: { returnBody: string[], declarations: string[] }, path: string) {
        if (property.isUnmapped == true) {
            return;
        }
    ...
    private _appendPrepare(property: SchemaBase<any, any>, builder: FunctionBuilder<"return" | "functions">, value: string, selectorPath?: string) {
    ...
    private _appendDeserializer(property: SchemaBase<any, any>, builder: FunctionBuilder<"return" | "variables">, value: string, selectorPath?: string) {
    ...
    private _appendHashMapper(property: SchemaBase<any, any>, builder: FunctionBuilder<"return" | "functions">, selectorPath: string) {
        ...
        builder.append("return", `$\{${optionalPath.join("?.")}\}`);
    }
    ...
    private _appendMerge(property: SchemaBase<any, any>, builder: FunctionBuilder<"assignments" | "functions" | "deserializers" | "post-ifs">, value: string, selectorPath: string) {
        ...
        if (property.isNullable === true || property.isOptional) {
        ...
        builder.append("assignments", `${["destination", ...split].join(".")} = ${["source", ...split].join("?.")}`);
    }

    private _appendEnricher(property: SchemaBase<any, any>, builder: FunctionBuilder<"variables" | "enrichments" | "functions" | "entity">, value: string, selectorPath: string) {
    ...
    compile(): CompiledSchema<T> {
        ...
        const hashBuilder = new FunctionBuilder().use("return").use("functions");
        ...
        this._iterate(schema, (name, property, parentName, _, parents) => {
            const isNested = !!parentName;

            if (parentName == null) {
                allPropertyNamesAndPaths.push(name);
            } else {
                allPropertyNamesAndPaths.push(`${parentName}.${name}`);
            }

            // Check if the property or any parent is nullable/optional
            const isParentNullableOrOptional = parents?.some((parent) => parent.isNullable || parent.isOptional) || false;
            ...
            const selectorPath = parents && parents.length > 0
                ? `entity${parents.map((_, idx) => parents.slice(0, idx + 1).map(p => `${isParentNullableOrOptional ? "?" : ""}.${Object.keys(p)[0]}`).join("")).join("")}${isParentNullableOrOptional ? "?" : ""}.${name}`
                : `entity${isPropertyNullableOrOptional ? "?" : ""}.${name}`;

            if (isNested) {
                // Skip adding nested properties at the root level or unmapped properties
                return;
            }
            ...
            if (property.type === SchemaTypes.Object) {
                ...
                // Traverse nested properties inline
                const nestedProperties = Object.keys(property.instance);

                nestedProperties.forEach((nestedKey, index) => {

                    const nestedProperty = property.instance[nestedKey];
                    const nestedSelectorPath = `${selectorPath}${isPropertyNullableOrOptional ? "?" : ""}.${nestedKey}`;
                    ...
                    if (nestedProperty.type === SchemaTypes.Object) {
                        ...
                        const nestedAssignment = `
                            ${nestedKey}: ${isNestedNullableOrOptional ? `${nestedSelectorPath} ? {` : `{`}`;
                        ...
                        const deepKeys = Object.keys(nestedProperty.instance);
                        deepKeys.forEach((deepKey, deepIndex) => {
                            const deepProperty = nestedProperty.instance[deepKey];
                            const deepSelectorPath = `${nestedSelectorPath}${isNestedNullableOrOptional ? "?" : ""}.${deepKey}`;

                            const deeplyNestedAssignment = `
                                ${deepKey}: ${isNestedNullableOrOptional ? `${deepSelectorPath} ?? null` : deepSelectorPath}${deepIndex < deepKeys.length - 1 ? "," : ""}`;
                            ...
                            this._appendEnricher(deepProperty, enrichFunciton, `
                                ${deepKey}: ${this._resolvePropertyValue(`${isNestedNullableOrOptional ? `${deepSelectorPath} ?? null` : deepSelectorPath}`, deepProperty)}${deepIndex < deepKeys.length - 1 ? "," : ""}`, deepSelectorPath);
                        ...
                        const nestedClosure = `}${isNestedNullableOrOptional ? ` : null` : ""}${index < nestedProperties.length - 1 ? "," : ""}`;
                    ...
                    const assignment = `
                        ${nestedKey}: ${isNestedNullableOrOptional ? `${nestedSelectorPath} ?? null` : nestedSelectorPath}${index < nestedProperties.length - 1 ? "," : ""}`;
                    ...
                    this._appendEnricher(nestedProperty, enrichFunciton, `
                            ${nestedKey}: ${this._resolvePropertyValue(`${isNestedNullableOrOptional ? `${nestedSelectorPath} ?? null` : nestedSelectorPath}`, nestedProperty)}${index < nestedProperties.length - 1 ? "," : ""}`, nestedSelectorPath);
                ...
                const closureEnd = `}${isPropertyNullableOrOptional ? ` : null` : ""}${nestedProperties.length > 0 ? "," : ""}`;
            ...
            this._appendEnricher(property, enrichFunciton, `
             ${name}: ${this._resolvePropertyValue(`${isPropertyNullableOrOptional ? `${selectorPath} ?? null` : selectorPath}`, property)},`, selectorPath);
            this._appendMerge(property, mergeFunciton, primitiveAssignment, selectorPath);
            this._appendCompare(property, compareFunction, selectorPath);
        });

        const prepareFunctionBody = `${prepareBuilder.join("functions", "")} return { ${prepareBuilder.join("return", "")} };`;
        ...
        return \`${hashBuilder.join("return", "")}\`;`
        console.log('mergeFunctionBody', mergeFunctionBody)
        ...
        const toHash = Function("entity", hashFunctionBody) as (entity: NonNullEntity<T>) => string;
